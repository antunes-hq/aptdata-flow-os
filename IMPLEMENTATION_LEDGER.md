...
| F0.5 | 0 | Contrato de Squad Confiável, WorkPacket, EvidenceRecord, JudgeResult e MaestroDecision | in_progress | `docs/contracts/squad-confiavel-workpacket.md`; `schemas/governance/`; `aptdata/governance/` | modelos + schemas + store SQLite append-only + gates + `GovernanceJudge` read-only + ensaio; 945 passed/10 skipped; Judge GO independente, Maestro approve, 10 persistidos/7 recuperados | integrar hooks de execução/registro automático; ainda sem autonomia e sem My Universe real |
