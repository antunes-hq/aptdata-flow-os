...
| F0.5 | 0 | Contrato de Squad Confiável, WorkPacket, EvidenceRecord, JudgeResult e MaestroDecision | in_progress | `docs/contracts/squad-confiavel-workpacket.md`; `schemas/governance/`; `aptdata/governance/`; `aptdata/events/models.py`; `tests/test_governance_real_flow*.py` | ensaio com `BronzeFlow` real e falha real de `SilverFlow`; eventos started→completed/failed, EvidenceRecord, Judge GO, recuperação SQLite; 955 passed/10 skipped | revisão final do contrato + decidir próximo gate; sem autonomia/My Universe real |
