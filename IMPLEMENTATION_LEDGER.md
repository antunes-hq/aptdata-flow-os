...
| F0.5 | 0 | Contrato de Squad Confiável, WorkPacket, EvidenceRecord, JudgeResult e MaestroDecision | in_progress | `docs/contracts/squad-confiavel-workpacket.md`; `schemas/governance/`; `aptdata/governance/` | modelos + schemas + store SQLite append-only + gates + ensaio read-only; 938 passed/10 skipped; runtime rehearsal: 10 persistidos, 7 recuperados, Judge GO independente, Maestro approve, integrated | Judge operacional real e integração com execução ainda pendentes |
