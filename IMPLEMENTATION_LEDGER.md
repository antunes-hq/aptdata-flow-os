...
| F0.5 | 0 | Contrato de Squad Confiável, WorkPacket, EvidenceRecord, JudgeResult e MaestroDecision | in_progress | `docs/contracts/squad-confiavel-workpacket.md`; `schemas/governance/`; `aptdata/governance/` | modelos + schemas + store + gates + `GovernanceJudge` + hook opcional no `Workflow`; 948 passed/10 skipped; runtime PASS/FAIL registrado sem payload | integrar event envelope/run_id universal e ensaio com Flow real read-only; sem autonomia/My Universe real |
