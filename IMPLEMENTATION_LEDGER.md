...
| F0.5 | 0 | Contrato de Squad Confiável, WorkPacket, EvidenceRecord, JudgeResult e MaestroDecision | in_progress | `docs/contracts/squad-confiavel-workpacket.md`; `schemas/governance/`; `aptdata/governance/`; `aptdata/events/models.py` | FlowEvent durável + `run_id` correlaciona started→completed/failed→EvidenceRecord→WorkPacket; 951 passed/10 skipped; sem payload bruto | ensaio com Flow real read-only e revisão de contrato; sem autonomia/My Universe real |
