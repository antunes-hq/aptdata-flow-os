...
| F0.5 | 0 | Contrato de Squad Confiável, WorkPacket, EvidenceRecord, JudgeResult e MaestroDecision | in_progress | `docs/contracts/squad-confiavel-workpacket.md`; `schemas/governance/` | contrato v0.1 + modelos Pydantic + store SQLite append-only + gates puros; 936 passed/10 skipped; Maestro aprovou execução inicial | implementar integração de execução/registro e ensaio read-only; Judge operacional ainda pendente |
