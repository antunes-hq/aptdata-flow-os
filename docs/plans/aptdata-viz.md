...
- **aptdata/tui/monitor.py**: `_DAGPanel` (grafo do flow), `_StatusTable` (status por step), barra de memória — já é uma "viz" no terminal.
...
**Princípio:** aptdata-viz é **consumidor**, não dono de lógica. Zero regra de negócio no viz — ele só lê a mesma fonte da observability e do registry e renderiza. Absorve o painel multiverso e espelha a TUI (paridade web).
...
## Arquitetura
- **Backend**: novo módulo `aptdata/viz/` — servidor (FastAPI, já usado no multiverso; ou stdlib pra manter leve) expondo API de LEITURA:
  ...
  - CLI: `aptdata viz` sobe o servidor (paridade com `aptdata monitor` da TUI).
- **Frontend**: single-page **vanilla JS** (mesmo estilo leve do painel/Aconchego, sem build), DAG com **Mermaid** ou **vis-network** via CDN, live via **EventSource (SSE)**. Uma viz, dois frontends (TUI no terminal + web no browser) sobre a MESMA API.
- **Deploy**: Docker na VPS atrás do Traefik (como o painel), domínio `aptdata-viz.srv1723096.hstgr.cloud` — **substitui** o multiverso painel a médio prazo.
...
1. **Backend read API** (`aptdata/viz`, FastAPI) sobre o event store + registry MCP. CLI `aptdata viz`. Deploy Docker/Traefik.
...
- **Fable 5**: o viz é agnóstico de modelo, mas expõe o `modelo` por agente — útil pra ver quem está no `claude-fable-5` no teste de fluxo geral.
