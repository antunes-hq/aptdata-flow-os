...
from smart_data.plugins.local_fs import CSVReader
...
from smart_data.plugins.dataset import InMemoryDataset
from smart_data.plugins.local_fs import ParquetWriter
...
A Bronze -> Silver -> Gold data lakehouse pipeline built with smart-data.
...
def _scaffold_medallion(project_name: str, project_dir: Path) -> None:
    ...
    (project_dir / "smart-data.yaml").write_text(
...
A data quality enforcement pipeline using smart-data schema contracts
and expectations.
...
def scaffold(
    ...
    """Gera um projeto smart-data a partir de um template."""
