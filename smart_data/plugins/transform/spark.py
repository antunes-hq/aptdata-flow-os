...
class PySparkTransformer(BaseTransformer):
    ...
    def transform(self, data: Any) -> Any:
        ...
        with tracer.start_as_current_span(self._name) as span:
            ...
            span.set_attribute("smart_data.transformer.engine", "pyspark")
            span.set_attribute("smart_data.transformer.rows_in", rows_in)
            span.set_attribute("smart_data.spark.app_name", self._app_name)

            # Try to capture Spark UI URL.
            try:
                ui_url = spark.sparkContext.uiWebUrl
                if ui_url:
                    span.set_attribute("smart_data.spark.ui_url", ui_url)
