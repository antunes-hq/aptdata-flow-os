...
from smart_data.plugins.quality.expectations import BaseExpectation
...
class QualityValidator:
    ...
    def validate(self, data: Any) -> Any:
        ...
        This method is compatible with :meth:`~smart_data.core.workflow.Workflow.add_step`.
        ...
        data:
            ...
            :class:`~smart_data.plugins.dataset.InMemoryDataset`, or
            ``list[dict]``.
        ...
        tracer = get_tracer("smart_data.quality")
        with tracer.start_as_current_span(self.name) as span:
            span.set_attribute("smart_data.quality.validator_name", self.name)
            span.set_attribute("smart_data.quality.enforcement", self.enforcement.value)
            span.set_attribute("smart_data.quality.num_expectations", len(self.expectations))
            ...
            span.set_attribute("smart_data.quality.passed", passed)
            span.set_attribute("smart_data.quality.num_checks", len(report.checks))
            failed_count = report.summary.get(CheckStatus.FAILED, 0)
            span.set_attribute("smart_data.quality.failed_checks", failed_count)
